  ...
  const action = async () => {
    if (isSignatureAction) {
      ...
      txs = await delegateTokensBySig({
        user,
        tokens: [governanceConfig.colendTokenAddress, governanceConfig.esColendTokenAddress],
        data: [
          {
            delegatee,
            nonce: seamNonce,
  ...
  const signMetaTxs = async () => {
    if (delegationTokenType === DelegationTokenType.BOTH) {
      ...
      const [seamNonce, esSEAMNonce] = await Promise.all([
        getTokenNonce(user, governanceConfig.colendTokenAddress),
        getTokenNonce(user, governanceConfig.esColendTokenAddress),
      ]);
      ...
      const txs = [
        {
          ...
          governanceToken: governanceConfig.colendTokenAddress,
        ...
        },
        {
          ...
          governanceToken: governanceConfig.esColendTokenAddress,
        ...
        txs = await delegate({
          ...
          governanceToken:
            delegationTokenType === DelegationTokenType.SEAM
              ? governanceConfig.colendTokenAddress
              : governanceConfig.esColendTokenAddress,
        });
        setActionTx(txs[0]);
