...
export interface ModalContextType<T extends ModalArgsType> {
  ...
  openGovVote: () => void;
  openClaimSuccess: () => void;
  openClaimFail: (text1: string, text2: string) => void;
  openManageRedeem: (redeem: RedeemQueueType) => void;
  openConfirmRedeem: () => void;
  openAddRewards: (poolName: string, poolContract: string) => void;
  openDaoSuccess: (text1: string, text2: string) => void;
  openDaoFail: () => void;
  openStartEpoch: (content: string) => void;
  close: () => void;
  type?: ModalType;
  message?: MessageType;
  redeemSelected?: RedeemQueueType;
  args: T;
...
export const ModalContextProvider: React.FC = ({ children }) => {
  ...
  const [message, setMessage] = useState<MessageType>({ text1: '', text2: '' });
  const [redeemSelected, setRedeemSelected] = useState<RedeemQueueType>();
  const [contentNewEpoch, setContentNewEpoch] = useState('');
  ...
  const [poolVoter, setPoolVoter] = useState({
    poolName: '',
    poolContract: '',
  });
  ...
  return (
    <ModalContext.Provider
      value={{
        ...
        openClaimFail: (text1: string, text2?: string) => {
          setType(ModalType.ClaimFail);
          setMessage({
            text1: text1,
            text2: text2,
          });
        },
        openManageRedeem: (redeem: RedeemQueueType) => {
          setRedeemSelected(redeem);
          setType(ModalType.ManageRedeem);
        },
        openConfirmRedeem: () => {
          setType(ModalType.ConfirmRedeem);
        },
        openAddRewards: (poolName: string, poolContract: string) => {
          setPoolVoter({
            poolName: poolName,
            poolContract: poolContract,
          });
          setType(ModalType.AddRewards);
        },
        openDaoSuccess: (text1: string, text2?: string) => {
          setMessage({
            text1: text1,
            text2: text2,
          });
          setType(ModalType.DaoSuccess);
        },
        openDaoFail: () => {
          setType(ModalType.DaoFail);
        },
        openStartEpoch: (content: string) => {
          setContentNewEpoch(content);
          setType(ModalType.NewEpoch);
        },
        close: () => {
          setType(undefined);
          setArgs({});
