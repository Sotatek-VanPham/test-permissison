...
import { API_ETH_MOCK_ADDRESS, ReserveDataHumanized } from 'colend-contract-helpers';
import {
  ComputedUserReserve,
...
export interface AppDataContextType {
  ...
  permissionSupply: Record<string, boolean>;
  permissionBorrow: Record<string, boolean>;
}
...
export const AppDataProvider: React.FC = ({ children }) => {
  ...
  const [permissionSupplyObj, setPermissionSupply] = useState({});
  const [permissionBorrowObj, setPermissionBorrow] = useState({});

  const handlePermission = async () => {
    if (formattedPoolReserves && formattedPoolReserves.length > 0) {
      const arrayAssets = formattedPoolReserves.map((item) => item.underlyingAsset);
      if (soulBoundTokenContract && currentAccount) {
        const [arrayPermissionSupply, arrayPermissionBorrow] = await Promise.all([
          soulBoundTokenContract.hasDepositorSBTs(currentAccount, arrayAssets),
          soulBoundTokenContract.hasBorrowerSBTs(currentAccount, arrayAssets),
        ]);

        const permissionSupply = arrayAssets.reduce((obj: any, key, index) => {
          obj[key] = arrayPermissionSupply[index];
          return obj;
        }, {});

        const permissionBorrow = arrayAssets.reduce((obj: any, key, index) => {
          obj[key] = arrayPermissionBorrow[index];
          return obj;
        }, {});

        const WCORE = formattedPoolReserves.filter((item) => item.symbol === 'WCORE');
        if (WCORE.length > 0) {
          setPermissionSupply({
            ...permissionSupply,
            [API_ETH_MOCK_ADDRESS.toLowerCase()]: permissionSupply[WCORE[0].underlyingAsset],
          });
          setPermissionBorrow({
            ...permissionBorrow,
            [API_ETH_MOCK_ADDRESS.toLowerCase()]: permissionBorrow[WCORE[0].underlyingAsset],
          });
        }
      }
    }
  ...
  useEffect(() => {
    ...
    formattedPoolReserves && formattedPoolReserves.length > 0,
  ...
  return (
    <AppDataContext.Provider
      value={{
        ...
        permissionSupply: permissionSupplyObj,
        permissionBorrow: permissionBorrowObj,
        eModes,
        user: {
          ...user,
