...
import { normalize, valueToBigNumber } from 'colend-math-utils';
...
export class GovernanceService implements Hashable {
  ...
  constructor(provider: Provider, public readonly chainId: number) {
    ...
    this.seam = COLEND__factory.connect(governanceConfig.colendTokenAddress, this.provider);
    this.esSEAM = vCOLEND__factory.connect(governanceConfig.esColendTokenAddress, this.provider);
