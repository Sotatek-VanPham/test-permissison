...
import { normalizeBN, RAY, rayDiv, rayMul } from 'colend-math-utils';
...
const getLiquidityRate = (d: Rate) => d.liquidityRate;
...
function getRates({
  ...
  for (let i = 0; i <= resolution; i++) {
    ...
    // When hovering below optimal utilization rate, actual data
      ...
      const overallBorrowRate = new BigNumber(totalDebt).eq(0)
        ? new BigNumber(0)
        : new BigNumber(totalVariableDebt)
            .times(new BigNumber(theoreticalVariableAPY))
            .plus(new BigNumber(totalStableDebt).times(new BigNumber(theoreticalStableAPY)))
            .div(totalDebt);

      const theoreticalSupplyAPY = overallBorrowRate.times(new BigNumber(utilization)).toFixed(2);
      rates.push({
        stableRate: theoreticalStableAPY,
        variableRate: theoreticalVariableAPY,
        utilization,
        liquidityRate: Number(theoreticalSupplyAPY),
      });
    }
        ...
        <svg width={width} height={height}>
          <Group left={margin.left} top={margin.top}>
            ...
            <LinePath
              ...
              data={data}
              x={(d) => dateScale(getDate(d)) ?? 0}
              y={(d) => yValueScale(getLiquidityRate(d)) ?? 0}
              curve={curveMonotoneX}
            />
            ...
            {tooltipData && (
              <g>
                ...
                <circle
                  cx={tooltipLeft}
                  cy={yValueScale(getLiquidityRate(tooltipData)) + 1}
                  r={4}
                  fill="black"
                  fillOpacity={0.1}
                  stroke="black"
                ...
                <circle
                  cx={tooltipLeft}
                  cy={yValueScale(getLiquidityRate(tooltipData))}
                  r={4}
                  fill={accentColorDark}
                  stroke="white"
                  strokeWidth={2}
